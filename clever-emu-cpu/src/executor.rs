...
use clever_emu_mem::{bus::{GlobalMemory, LocalMemory}, io::IoBus};

use crate::cpu::Cpu;
use crate::cpu::CpuMemorySizes;

use clever_emu_mem::{cache::CacheLine, phys::{Page, SysMemory}};
...
fn process_exceptions(cpu: &mut Cpu, f: impl FnOnce(&mut Cpu) -> CPUResult<()>) -> CPUResult<()>{
    match f(cpu){
        ...
        Err(e) => {
            match cpu.handle_exception(e){
                Ok(()) => Ok(()),
                Err(_) if e == CPUException::Abort => Err(CPUException::Reset),
                Err(CPUException::Reset) => Err(CPUException::Reset),
                Err(_) => {
                    cpu.handle_exception(CPUException::Abort).map_err(|_| CPUException::Reset)
                }
            }
        }
    }
...
impl CpuExecutor{
    ...
    pub fn new(num_cpus: NonZero<usize>, memory_sizes: CpuMemorySizes, iobus: IoBus) -> Self{
        ...
        let sys_mem = SysMemory::new((memory_sizes.sys_mem_size / (Page::SIZE as usize)).try_into().expect("The maximum size of system memory is 16TiB (16384 GiB)"));

        let l3cache = Arc::new(GlobalMemory::new(sys_mem, memory_sizes.l3size / (CacheLine::SIZE as usize)));

        let iobus = Arc::new(iobus);

        let cpus = (0..num_cpus.get()).map(|_| Cpu::new(l3cache.clone(), iobus.clone(), memory_sizes)).collect::<Vec<_>>();
        cpus[0].enable(true);

        Self{cpus, l3cache, iobus}
    }
